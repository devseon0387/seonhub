...
export default function StrategyPage() {
  ...
  const cancel = () => { setIsAdding(false); setNewName(''); setNewIcon('Folder'); setNewColor('#1e3a8a'); setPickerOpen(false); };
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: newName.trim() ? '#1e3a8a' : '#e8e3df',
                  color: newName.trim() ? '#fff' : '#b8afa8',
                  border: 'none', fontSize: '13px', fontWeight: 600,
                  cursor: newName.trim() ? 'pointer' : 'not-allowed',
                  transition: 'background 0.12s',
                }}
                onMouseEnter={e => { if (newName.trim()) e.currentTarget.style.background = '#1e40af'; }}
                onMouseLeave={e => { if (newName.trim()) e.currentTarget.style.background = '#1e3a8a'; }}
              >
                만들기
              </button>
            </div>
