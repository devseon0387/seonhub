...
import { ClipboardCheck, ChevronLeft, ChevronRight, Receipt } from 'lucide-react';
...
export default function ManagerSettlementPage() {
  ...
  // 매니저별 정산: paymentDueDate 기준으로 해당 월 에피소드 필터링
  const summaries: ManagerSummary[] = useMemo(() => {
    return partners
      .map(manager => {
        // 이 매니저가 참여한 모든 프로젝트
  ...
  const grandTotal = summaries.reduce((s, ms) => s + ms.totalAmount, 0);
  const grandPaid = summaries.reduce((s, ms) => s + ms.paidAmount, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">매니저를 선택해 정산 내역을 확인하세요</p>
        </div>
            ...
            <div className="min-w-[700px]">
              ...
              {summaries.length === 0 ? (
                ...
                <div className="divide-y divide-gray-100">
                  {summaries.map(({ manager, projectCount, episodeCount, totalAmount, unpaidAmount, nearestDueDate }) => (
                    <Link
                      key={manager.id}
                      href={`/finance/manager-settlement/${manager.id}?year=${selectedDate.year}&month=${selectedDate.month}`}
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <span className="text-sm font-semibold text-gray-900 truncate">{manager.name}</span>
                      </div>
                      <span className="text-sm text-gray-500">{manager.jobTitle ?? '-'}</span>
